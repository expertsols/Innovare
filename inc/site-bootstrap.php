...
define( 'INNOVARE_SITE_BOOTSTRAP_VERSION', 8 );

/** Stable English menu name so production always resolves the same nav menu. */
define( 'INNOVARE_PRIMARY_MENU_NAME', 'Innovare Primary' );
 ...
 */
function innovare_core_page_definitions() {
	return array(
		'home'                  => array(
...
/**
 * Display name for Innovare (site brand).
 *
 * @return string
 */
function innovare_brand_name() {
	return innovare_theme_name();
}

/**
 * Update Customizer-stored phone numbers when still using the legacy default.
 */
function innovare_migrate_contact_phone_defaults() {
	if ( wp_installing() || get_option( 'innovare_contact_phone_migrated_v2', false ) ) {
		return;
	}

	$old_phone = '+92 345 4243541';
	$new_phone = innovare_default_contact_phone();
	$keys      = array( 'innovare_contact_phone', 'innovare_contact_whatsapp' );
	...
	update_option( 'innovare_contact_phone_migrated_v2', true, false );
}

/**
 * Migrate hyphenated number (+92 333-4106911) to spaced format.
 */
function innovare_migrate_contact_phone_no_hyphen() {
	if ( wp_installing() || get_option( 'innovare_contact_phone_migrated_v3', false ) ) {
		return;
	}
	...
	update_option( 'innovare_contact_phone_migrated_v3', true, false );
}

/**
 * Migrate compact number (+92 3334106911) to spaced format (+92 333 4106911).
 */
function innovare_migrate_contact_phone_spaced_format() {
	if ( wp_installing() || get_option( 'innovare_contact_phone_migrated_v4', false ) ) {
		return;
	}
	...
	update_option( 'innovare_contact_phone_migrated_v4', true, false );
}

/**
 * Run all one-time data migrations (phone numbers, slugs, etc.).
 */
function innovare_migrate_about_slug() {
	if ( wp_installing() || get_option( 'innovare_about_slug_migrated_v3', false ) ) {
		return;
	}
	...
	update_option( 'innovare_about_slug_migrated_v3', true, false );
}

function innovare_bootstrap_run_migrations() {
	innovare_migrate_legacy_storage_prefix();
	innovare_migrate_about_slug();
	innovare_migrate_legacy_about_page_slugs();
	innovare_migrate_legacy_site_branding();
	innovare_migrate_legacy_branding();
	innovare_migrate_legacy_footer_widgets();
	innovare_migrate_contact_phone_defaults();
	innovare_migrate_contact_phone_no_hyphen();
	innovare_migrate_contact_phone_spaced_format();
}
 ...
 */
function innovare_seed_customizer_defaults() {
	if ( wp_installing() ) {
		return 0;
	}

	$written = 0;
	foreach ( innovare_theme_mod_defaults() as $key => $default ) {
 ...
 */
function innovare_sync_core_page_templates( $force = false ) {
	if ( wp_installing() ) {
		return;
	}

	foreach ( innovare_core_page_definitions() as $slug => $def ) {
		...
		$is_bootstrap     = (bool) get_post_meta( $post_id, '_innovare_bootstrap_page', true );

		if ( $force || $is_bootstrap || ! $current_template || 'default' === $current_template ) {
			update_post_meta( $post_id, '_wp_page_template', $def['template'] );
			update_post_meta( $post_id, '_innovare_bootstrap_page', 'v1' );
		}
	}
}
 ...
 */
function innovare_bootstrap_legal_and_solutions() {
	if ( function_exists( 'innovare_migrate_privacy_page_slug' ) ) {
		innovare_migrate_privacy_page_slug();
	}
	if ( function_exists( 'innovare_ensure_legal_pages' ) ) {
		innovare_ensure_legal_pages();
	}
	if ( function_exists( 'innovare_ensure_solution_detail_pages' ) ) {
		innovare_ensure_solution_detail_pages();
	}
	if ( function_exists( 'innovare_sync_privacy_policy_option' ) ) {
 ...
 */
function innovare_clear_theme_runtime_cache() {
	if ( function_exists( 'innovare_clear_solutions_cache' ) ) {
		innovare_clear_solutions_cache();
	}
	if ( function_exists( 'innovare_clear_team_cache' ) ) {
 ...
 * @return string[]
 */
function innovare_legacy_about_page_slugs() {
 ...
 */
function innovare_migrate_legacy_about_page_slugs() {
	if ( wp_installing() || get_option( 'innovare_legacy_about_slug_migrated_v2', false ) ) {
		return;
	}
	...
	foreach ( innovare_legacy_about_page_slugs() as $slug ) {
		if ( $slug === $canonical ) {
			continue;
		}
		...
		if ( ! $canonical_page ) {
			...
			update_post_meta( (int) $page->ID, '_innovare_bootstrap_page', 'v1' );
	...
	innovare_fix_legacy_about_menu_urls();
	update_option( 'innovare_legacy_about_slug_migrated_v2', true, false );
}

/**
 * Replace legacy site title/branding left from the previous site.
 */
function innovare_migrate_legacy_site_branding() {
	if ( wp_installing() || get_option( 'innovare_site_branding_migrated_v1', false ) ) {
		return;
	}

	$name = (string) get_option( 'blogname', '' );
	if ( '' === $name || false !== stripos( $name, 'Top Notch' ) ) {
		update_option( 'blogname', innovare_brand_name() );
	}
	...
	update_option( 'innovare_site_branding_migrated_v1', true, false );
}

/**
 * Migrate site title from legacy placeholder names to Innovare.
 */
function innovare_migrate_legacy_branding() {
	if ( wp_installing() || get_option( 'innovare_site_branding_migrated_v3', false ) ) {
		return;
	}

	$name    = (string) get_option( 'blogname', '' );
	$markers = array( 'andromeda', 'innovate', 'top notch' );
	$update  = ( '' === $name );

	if ( ! $update && $name ) {
		foreach ( $markers as $marker ) {
			if ( false !== stripos( $name, $marker ) ) {
				$update = true;
				break;
			}
		}
	...
	update_option( 'innovare_site_branding_migrated_v3', true, false );
}

/**
 * Remove legacy footer widgets that override theme footer columns.
 */
function innovare_migrate_legacy_footer_widgets() {
	if ( wp_installing() ) {
		return;
	}

	if ( get_option( 'innovare_footer_widgets_migrated_v2', false ) ) {
		return;
	}

	innovare_clear_footer_sidebars();
	update_option( 'innovare_footer_widgets_migrated_v2', true, false );
	update_option( 'innovare_footer_widgets_migrated_v1', true, false );
}

/**
 * Update menu items that still point at legacy About URLs or page IDs.
 */
function innovare_fix_legacy_about_menu_urls() {
	$canonical      = innovare_about_page_slug();
	$canonical_page = get_page_by_path( $canonical, OBJECT, 'page' );
	$canonical_url  = $canonical_page ? get_permalink( $canonical_page ) : innovare_page_url( $canonical );
	...
	foreach ( wp_get_nav_menus() as $menu ) {
		...
		foreach ( $items as $item ) {
			if ( 'post_type' === $item->type && 'page' === $item->object ) {
				...
				if ( $page instanceof WP_Post && in_array( $page->post_name, innovare_legacy_about_page_slugs(), true ) && $page->post_name !== $canonical ) {
					if ( $canonical_page ) {
			...
			$new_url = $item->url;
			foreach ( innovare_legacy_about_page_slugs() as $legacy_slug ) {
 ...
 */
function innovare_ensure_core_site_pages() {
	if ( wp_installing() ) {
		return array( 'created' => array(), 'existing' => array() );
	}

	$definitions = innovare_core_page_definitions();
 ...
 */
function innovare_configure_reading_settings( $force = false ) {
	if ( wp_installing() ) {
		return;
	}
 ...
 */
function innovare_maybe_set_default_site_title() {
	if ( wp_installing() ) {
		return;
	}
 ...
 */
function innovare_primary_menu_item_definitions() {
	return array(
 ...
 */
function innovare_get_or_create_primary_menu() {
	$menu = wp_get_nav_menu_object( INNOVARE_PRIMARY_MENU_NAME );
	...
	$menu_id = (int) wp_create_nav_menu( INNOVARE_PRIMARY_MENU_NAME );
 ...
 * @return bool
 */
function innovare_primary_menu_needs_sync( $menu_id ) {
	if ( ! $menu_id ) {
		return true;
	}
	...
	$expected = innovare_primary_menu_item_definitions();
 ...
 * @param int $menu_id Menu term ID.
 */
function innovare_sync_primary_menu_items( $menu_id ) {
	if ( ! $menu_id ) {
		return;
	}
	...
	$order = 0;
	foreach ( innovare_primary_menu_item_definitions() as $slug => $label ) {
		$page = get_page_by_path( $slug, OBJECT, 'page' );
		if ( ! $page || 'publish' !== $page->post_status ) {
			continue;
		}
 ...
 * @return int Menu term ID or 0.
 */
function innovare_ensure_primary_navigation_menu( $force_sync = false ) {
	if ( wp_installing() ) {
		return 0;
	}

	$menu_id = innovare_get_or_create_primary_menu();
	if ( ! $menu_id ) {
		return 0;
	}
	...
	$should_sync = $force_sync
		...
		|| innovare_primary_menu_needs_sync( $menu_id );

	if ( $should_sync ) {
		innovare_sync_primary_menu_items( $menu_id );
		$locations            = get_theme_mod( 'nav_menu_locations', array() );
 ...
 */
function innovare_run_site_bootstrap( $flush_rewrites = true ) {
	$messages     = array();
	$prev_version = (int) get_option( 'innovare_site_bootstrap_version', 0 );

	innovare_bootstrap_run_migrations();

	$customizer_seeded = innovare_seed_customizer_defaults();
	if ( $customizer_seeded > 0 ) {
		$messages[] = __( 'Customizer contact and social defaults applied.', 'innovare' );
	}

	$pages = innovare_ensure_core_site_pages();
	innovare_sync_core_page_templates( true );
	innovare_bootstrap_legal_and_solutions();

	$solutions_seed = innovare_seed_solutions_data( false );
	if ( ! empty( $solutions_seed['updated'] ) ) {
		$messages[] = __( 'Solution content synced to the database from theme defaults.', 'innovare' );
	}

	$team_seed = innovare_seed_team_data( false );
	...
	innovare_configure_reading_settings( true );
	innovare_maybe_set_default_site_title();
	$menu_id = innovare_ensure_primary_navigation_menu( true );
	...
	$messages[] = sprintf(
		...
		(int) get_option( INNOVARE_SOLUTIONS_VERSION_OPTION, 0 )
	);
	$messages[] = __( 'Legal pages (Privacy, Terms, Sitemap) ensured.', 'innovare' );

	innovare_clear_theme_runtime_cache();

	if ( $prev_version < INNOVARE_SITE_BOOTSTRAP_VERSION ) {
		$messages[] = sprintf(
			/* translators: %d: bootstrap version number */
			__( 'Bootstrap upgraded to version %d.', 'innovare' ),
			INNOVARE_SITE_BOOTSTRAP_VERSION
		);
	}

	update_option( 'innovare_site_bootstrap_version', INNOVARE_SITE_BOOTSTRAP_VERSION, false );
	update_option( 'innovare_site_bootstrap_last_run', time(), false );
 ...
 */
function innovare_on_theme_activation_bootstrap() {
	innovare_run_site_bootstrap( true );
}
add_action( 'after_switch_theme', 'innovare_on_theme_activation_bootstrap' );

/**
 * Auto-run bootstrap when the theme bootstrap version is bumped on deploy.
 */
function innovare_maybe_upgrade_site_bootstrap() {
	if ( wp_installing() ) {
		return;
	}

	$stored = (int) get_option( 'innovare_site_bootstrap_version', 0 );
	if ( $stored >= INNOVARE_SITE_BOOTSTRAP_VERSION ) {
		return;
	}

	innovare_run_site_bootstrap( true );
}
add_action( 'init', 'innovare_maybe_upgrade_site_bootstrap', 11 );

/**
 * Ensure core content exists on every load (same pattern as legal-pages.php).
 */
function innovare_bootstrap_site_content() {
	static $done = false;
	...
	innovare_bootstrap_run_migrations();
	innovare_seed_customizer_defaults();
	innovare_ensure_core_site_pages();
	innovare_sync_core_page_templates();
	innovare_bootstrap_legal_and_solutions();
	innovare_seed_solutions_data( false );
	innovare_seed_team_data( false );
	innovare_configure_reading_settings();
	innovare_ensure_primary_navigation_menu();
	innovare_clear_theme_runtime_cache();
}
add_action( 'init', 'innovare_bootstrap_site_content', 12 );

/**
 * Admin: Appearance → Site Setup — manual import / status.
 */
function innovare_site_setup_admin_menu() {
	add_theme_page(
		__( 'Innovare — Site Setup', 'innovare' ),
		__( 'Site Setup', 'innovare' ),
		'manage_options',
		'innovare-site-setup',
		'innovare_site_setup_admin_page'
	);
}
add_action( 'admin_menu', 'innovare_site_setup_admin_menu' );

/**
 * Render Site Setup admin page.
 */
function innovare_site_setup_admin_page() {
	...
	if ( isset( $_POST['innovare_reset_solutions'] ) && check_admin_referer( 'innovare_reset_solutions' ) ) {
		innovare_reset_solutions_data();
		$reset_solutions = true;
	}

	if ( isset( $_POST['innovare_reset_team'] ) && check_admin_referer( 'innovare_reset_team' ) ) {
		innovare_reset_team_data();
		$reset_team = true;
	}

	if ( isset( $_POST['innovare_run_bootstrap'] ) && check_admin_referer( 'innovare_site_bootstrap' ) ) {
		$results = innovare_run_site_bootstrap( true );
		$ran     = true;
	}
	...
	?>
	<div class="wrap">
		<h1><?php echo esc_html( sprintf( __( '%s — Site Setup', 'innovare' ), innovare_brand_name() ) ); ?></h1>
		<p class="description">
			<?php echo esc_html( innovare_theme_name() ); ?>
			<?php esc_html_e( 'WordPress theme by', 'innovare' ); ?>
			<a href="<?php echo esc_url( innovare_theme_author_uri() ); ?>" target="_blank" rel="noopener noreferrer"><?php echo esc_html( innovare_theme_author() ); ?></a>
			(<a href="<?php echo esc_url( innovare_theme_author_uri() ); ?>" target="_blank" rel="noopener noreferrer">expertsols.com</a>).
		</p>
		...
		<p class="description">
			<?php
			$last_run = get_option( 'innovare_site_bootstrap_last_run' );
			...
			printf(
				...
				(int) get_option( 'innovare_site_bootstrap_version', 0 ),
		...
		<ul style="list-style:disc;margin-left:1.5em;">
			<li><?php esc_html_e( 'Core pages with Innovare page templates', 'innovare' ); ?></li>
			...
			<li><?php esc_html_e( 'Legacy About page slug migration (company, about-us, older slugs → about)', 'innovare' ); ?></li>
			...
			<li><?php esc_html_e( 'Site title migration to Innovare when still using a placeholder or old brand name', 'innovare' ); ?></li>
			<li><?php esc_html_e( 'Legacy Customizer and database keys copied to innovare_*', 'innovare' ); ?></li>
		...
		<p class="description">
			...
			printf(
				...
				esc_html__( 'Stored version: %1$d · Theme defaults version: %2$d · Option key: innovare_team_data', 'innovare' ),
				(int) get_option( INNOVARE_TEAM_VERSION_OPTION, 0 ),
				(int) INNOVARE_TEAM_DATA_VERSION
			);
			?>
		</p>
		<?php
		$team_members = innovare_get_team_members( false );
		if ( $team_members ) :
			?>
		...
		<form method="post" style="margin-bottom:2em;">
			<?php wp_nonce_field( 'innovare_reset_team' ); ?>
			<p>
				<button type="submit" name="innovare_reset_team" class="button button-secondary" onclick="return confirm('<?php echo esc_js( __( 'Reset core team content to theme defaults? Custom database edits will be lost.', 'innovare' ) ); ?>');">
					<?php esc_html_e( 'Reset core team to theme defaults', 'innovare' ); ?>
				</button>
			</p>
			<p class="description"><?php esc_html_e( 'Powers the About page team section. Edit wp_options (innovare_team_data) or reset here after theme updates.', 'innovare' ); ?></p>
		</form>
		...
		<p class="description">
			...
			printf(
				...
				(int) get_option( INNOVARE_SOLUTIONS_VERSION_OPTION, 0 ),
				(int) INNOVARE_SOLUTIONS_DATA_VERSION
			);
			?>
		</p>
		<form method="post" style="margin-bottom:2em;">
			<?php wp_nonce_field( 'innovare_reset_solutions' ); ?>
			<p>
				<button type="submit" name="innovare_reset_solutions" class="button button-secondary" onclick="return confirm('<?php echo esc_js( __( 'Reset all solution content to theme defaults? Custom database edits will be lost.', 'innovare' ) ); ?>');">
					<?php esc_html_e( 'Reset solutions content to theme defaults', 'innovare' ); ?>
				</button>
			</p>
			<p class="description"><?php esc_html_e( 'Writes bundled defaults to wp_options (innovare_solutions_data). Use after a theme update when you want the latest solution copy and engagement steps.', 'innovare' ); ?></p>
		</form>

		<form method="post" style="margin-top:2em;">
			<?php wp_nonce_field( 'innovare_site_bootstrap' ); ?>
			<p>
				<button type="submit" name="innovare_run_bootstrap" class="button button-primary button-hero">
